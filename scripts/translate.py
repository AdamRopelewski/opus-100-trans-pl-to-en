...
from src.model.transformer_nmt import TransformerNMT, TransformerNMTConfig, beam_search_decode, greedy_decode
from src.utils.config import get_nested, load_config


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Translate PL text to EN with trained Transformer checkpoint.")
    parser.add_argument("--config", type=Path, default=Path("configs/project_config.yaml"), help="Path to project config YAML.")
    parser.add_argument("--checkpoint", type=Path, default=None, help="Model checkpoint path. Defaults to stage6_train.output_best_checkpoint.")
    parser.add_argument("--text", type=str, default=None, help="Source text. If omitted, text is read from stdin.")
    parser.add_argument("--decode", choices=("beam", "greedy"), default="beam", help="Decoding strategy.")
    parser.add_argument("--beam-size", type=int, default=None, help="Beam size. Defaults to stage7_eval.beam_size.")
    parser.add_argument("--length-penalty", type=float, default=None, help="Length penalty. Defaults to stage7_eval.length_penalty.")
    parser.add_argument("--max-new-tokens", type=int, default=None, help="Token limit. Defaults to stage7_eval.max_new_tokens.")
...
def _checkpoint_path(config_data: dict[str, Any], path: Path | None) -> Path:
    if path is not None:
        return path
    return Path(get_nested(config_data, "stage6_train.output_best_checkpoint", "checkpoints/model_best.pt"))
...
def _tokenizer_path(config_data: dict[str, Any], checkpoint: dict[str, Any]) -> Path:
    ...
    prefix = Path(get_nested(config_data, "stage3_tokenizer.model_prefix", "tokenizers/spm_pl_en"))
    return prefix.with_suffix(".model")


def _model_config(config_data: dict[str, Any], checkpoint: dict[str, Any], vocab_size: int, pad_id: int) -> TransformerNMTConfig:
...
def _translate_text(
    ...
    if len(src_tokens) > max_source_len:
        raise ValueError(f"Source too long: {len(src_tokens)} tokens > max_seq_len={max_source_len}")

    src_ids = torch.tensor([src_tokens], dtype=torch.long, device=device)
    if decode == "greedy":
        output_ids = greedy_decode(model, src_ids, int(tokenizer.bos_id()), int(tokenizer.eos_id()), max_new_tokens)
    else:
    ...
    special_ids = {int(tokenizer.pad_id()), int(tokenizer.bos_id()), int(tokenizer.eos_id())}
...
def _interactive_loop(
    ...
    while True:
        ...
        if not text:
            continue
        try:
            print(_translate_text(text, tokenizer, model, model_cfg, device, decode, max_new_tokens, beam_size, length_penalty))
...
def main() -> int:
    ...
    model_cfg = _model_config(config_data, checkpoint, int(tokenizer.vocab_size()), int(tokenizer.pad_id()))
    ...
    if max_new_tokens is None:
        max_new_tokens = int(get_nested(config_data, "stage7_eval.max_new_tokens", model_cfg.max_seq_len))
    ...
    if args.text is None and sys.stdin.isatty():
        return _interactive_loop(tokenizer, model, model_cfg, device, args.decode, max_new_tokens, beam_size, length_penalty)
    ...
    try:
        print(_translate_text(text, tokenizer, model, model_cfg, device, args.decode, max_new_tokens, beam_size, length_penalty))
    except ValueError as exc:
        print(str(exc), file=sys.stderr)
        return 3
    return 0
